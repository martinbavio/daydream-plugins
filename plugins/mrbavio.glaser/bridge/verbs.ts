...
function deliverableSection(input: PromptInput): string {
  ...
  return [
    `DELIVERABLE: ${variants} VARIANTS of the source, each a new draft viewport beside it, each a genuinely different direction the playbook allows — not ${variants} intensities of one idea. Decide the ${variants} directions first, in a sentence each. Then EACH VARIANT IS ONE ACTOR'S THREE CALLS, IN A ROW: draft_open {copyOf: "${id}", position: <its position below>, meta: {title: "${title} · ${spec.verb} n/${variants}", notes: <marker line, blank line, the direction>}} — a copy of the source beside it, the source untouched, answered without its page (you hold it); draft_replace {draft, target: <the target's id AS YOU READ IT FROM THE SOURCE — the copy answers to the source's ids>, element: <the target rewritten for that direction>} (or draft_set for the root's styles when the page itself is the target); draft_finalize, IMMEDIATELY — the canvas shows a draft as building until its own finalize lands it, so a finalize held back reads as work still going on. Never open the drafts up front for later; never hold finalizes for the end; never send the whole page. The notes' FIRST LINE is exactly \`${marker}\`; the canvas reads it to know the round, the user reads the rest while choosing. Positions: ${positions}. IN PARALLEL when the user allows sub-agents (a harness lets you spawn them only when the user, a CLAUDE.md or a skill asks — ask once if unsure): run the ${variants} variants as ${variants} sub-agents at once, and let each DESIGN ITS OWN — hand it the verb, the source viewport id "${id}", the target's id, its direction in a sentence, its position and its title, and have it call glaser_verb {verb: "${spec.verb}", viewport: "${id}", element: <target>, brief: <its direction>} itself for the playbook, read the target from get_viewport, write its own rewrite, and make its three calls — open, replace, finalize; the parent writes nothing and waits for all. The slow part is the writing; a sub-agent that only relays the parent's rewrite parallelizes nothing. Without sub-agents, do them one after another, each to its finalize before the next open. Either way the source stays untouched. When all ${variants} are on the canvas, report each direction in one line and call glaser_done; the user adopts one from the canvas (adopt, in a variant's title bar) or deletes the losers — do not remove anything.`,
  ].join("\n");
}
